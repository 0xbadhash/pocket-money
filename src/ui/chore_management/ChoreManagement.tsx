...
import type { ChoreDefinition, ChoreInstance, KanbanPeriod, MatrixKanbanCategory } from '../../types';
    ...
    >
      <div className="kid-kanban-board">
        ...
        <div className="matrix-kanban-body" style={{ display: 'flex', justifyContent: 'space-around', gap: '5px' }}>
          {visibleDates.map(date => (
            <DateColumnView key={date.toISOString()} date={date} onEditChore={handleEditChore} />
          ))}
        </div>
        {/* Add Chore Modal */}
        {showAddChoreModal && (
