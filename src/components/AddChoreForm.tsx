  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ccc', borderRadius: '5px' }}>
      <div>
        ...
        <input id="choreName" type="text" value={name} onChange={(e) => setName(e.target.value)} required style={{ width: '100%', padding: '8px', marginBottom: '10px' }} />
      </div>
      <div>
      ...
      <div>
        ...
        <input id="dueDate" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} required style={{ width: '100%', padding: '8px', marginBottom: '10px' }} />
      </div>
      <div>
        <label htmlFor="recurrenceType">Recurrence Type:</label>
        <select id="recurrenceType" value={recurrenceType} onChange={(e) => setRecurrenceType(e.target.value)} style={{ width: '100%', padding: '8px', marginBottom: '10px' }}>
      ...
      {recurrenceType === 'weekly' && (
        <div>
          ...
          <select id="weeklyDay" value={weeklyDay} onChange={(e) => setWeeklyDay(parseInt(e.target.value, 10))} style={{ width: '100%', padding: '8px', marginBottom: '10px' }}>
      ...
      {recurrenceType === 'monthly' && (
        <div>
          <label htmlFor="monthlyDay">Day of the Month (1-31):</label>
          <input id="monthlyDay" type="number" min="1" max="31" value={monthlyDay} onChange={(e) => setMonthlyDay(parseInt(e.target.value, 10))} style={{ width: '100%', padding: '8px', marginBottom: '10px' }} />
        </div>
      )}

      {recurrenceType === 'specificDays' && (
        <div>
          <label htmlFor="specificDaysInput">Specific Days (comma-separated, 0=Sun, 6=Sat):</label>
          <input id="specificDaysInput" type="text" value={specificDaysInput} onChange={(e) => setSpecificDaysInput(e.target.value)} placeholder="e.g., 1,3,5 for Mon,Wed,Fri" style={{ width: '100%', padding: '8px', marginBottom: '10px' }} />
        </div>
      )}
